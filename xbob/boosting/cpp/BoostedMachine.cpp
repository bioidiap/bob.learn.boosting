...
void BoostedMachine::add_weak_machine(const boost::shared_ptr<WeakMachine> weak_machine, const double weight){
...
void BoostedMachine::add_weak_machine(const boost::shared_ptr<WeakMachine> weak_machine, const blitz::Array<double,1> weights){
  m_weak_machines.push_back(weak_machine);
...
double BoostedMachine::forward(const blitz::Array<uint16_t,1>& features) const{
...
void BoostedMachine::forward(const blitz::Array<uint16_t,1>& features, blitz::Array<double,1> predictions) const{
...
void BoostedMachine::forward(const blitz::Array<uint16_t,2>& features, blitz::Array<double,1> predictions) const{
...
void BoostedMachine::forward(const blitz::Array<uint16_t,2>& features, blitz::Array<double,2> predictions) const{
  // initialize the predictions since they will be overwritten
...
void BoostedMachine::forward(const blitz::Array<uint16_t,2>& features, blitz::Array<double,1> predictions, blitz::Array<double,1> labels) const{
...
void BoostedMachine::forward(const blitz::Array<uint16_t,2>& features, blitz::Array<double,2> predictions, blitz::Array<double,2> labels) const{
  forward(features, predictions);
  // get the labels
  labels = -1;
  for (int i = predictions.extent(0); i--;){
...
static auto boostedMachine_doc = xbob::extension::ClassDoc(
  "BoostedMachine",
...
//  .add_prototype("weak_classifiers, weights", "")
  .add_prototype("hdf5", "")
//  .add_parameter("weak_classifiers", "[xbob.boosting.machine.WeakMachine]", "A list of weak machines that should be used in this strong machine")
//  .add_parameter("weights", "float <#machines,#outputs>", "The list of weights for the machines.")
  .add_parameter("hdf5", ":py:class:`xbob.io.HDF5File`", "The HDF5 file object to read the weak classifier from")
);
    ...
    switch (argument_count){
      case 1:{
        ...
        if (
          ...
          self->base.reset(new BoostedMachine(*file->f));
          return 0;
        }
      ...
      case 0:{
        self->base.reset(new BoostedMachine());
        return 0;
      }
      default:
        boostedMachine_doc.print_usage();
...
static auto boostedMachine_indices_doc = xbob::extension::VariableDoc(
  "indices",
...
static auto boostedMachine_weights_doc = xbob::extension::VariableDoc(
  "weights",
...
static auto boostedMachine_outputs_doc = xbob::extension::VariableDoc(
  "outputs",
  "int",
...
static auto boostedMachine_machines_doc = xbob::extension::VariableDoc(
  "weak_machines",
...
static auto boostedMachine_add_doc = xbob::extension::FunctionDoc(
  "add_weak_machine",
...
static auto boostedMachine_forward_doc = xbob::extension::FunctionDoc(
  "forward",
...
static auto boostedMachine_getIndices_doc = xbob::extension::FunctionDoc(
  "feature_indices",
...
static auto boostedMachine_load_doc = xbob::extension::FunctionDoc(
  "load",
...
.add_prototype("hdf5")
.add_parameter("hdf5", ":py:class:`xbob.io.HDF5File`", "The HDF5 file to load this machine from.")
;
...
static auto boostedMachine_save_doc = xbob::extension::FunctionDoc(
  "save",
...
.add_prototype("hdf5")
.add_parameter("hdf5", ":py:class:`xbob.io.HDF5File`", "The HDF5 file to save this weak machine to.")
;
