...
double StumpMachine::forward(const blitz::Array<double, 1>& features) const{
  return _predict(features((int)m_index));
}

void StumpMachine::forward(const blitz::Array<double, 2>& features, blitz::Array<double,1> predictions) const{
  for (int i = features.extent(0); i--;){
    predictions(i) = _predict(features(i, (int)m_index));
  }
}

void StumpMachine::forward(const blitz::Array<double, 2>& features, blitz::Array<double,2> predictions) const{
  for (int i = features.extent(0); i--;){
...
double StumpMachine::forward(const blitz::Array<uint16_t, 1>& features) const{
  return _predict(features((int)m_index));
}

void StumpMachine::forward(const blitz::Array<uint16_t, 2>& features, blitz::Array<double,1> predictions) const{
  for (int i = features.extent(0); i--;){
    predictions(i) = _predict(features(i, (int)m_index));
  }
}

void StumpMachine::forward(const blitz::Array<uint16_t, 2>& features, blitz::Array<double,2> predictions) const{
  for (int i = features.extent(0); i--;){
    predictions(i,0) = _predict(features(i, (int)m_index));
  }
}
...
static auto stumpMachine_doc = xbob::extension::ClassDoc(
  "StumpMachine",
    ...
    switch (argument_count){
      case 1:{
        ...
        if (
          ...
          self->base.reset(new StumpMachine(*file->f));
        } else {
      ...
      case 3:{
        ...
        if (
          ...
          self->base.reset(new StumpMachine(threshold, polarity, index));
        } else {
          stumpMachine_doc.print_usage();
          return -1;
        }
...
static auto stumpMachine_threshold_doc = xbob::extension::VariableDoc(
  "threshold",
  "float",
...
static auto stumpMachine_polarity_doc = xbob::extension::VariableDoc(
  "polarity",
  "float",
...
static auto stumpMachine_forward_doc = xbob::extension::FunctionDoc(
  "forward",
...
static auto stumpMachine_getIndices_doc = xbob::extension::FunctionDoc(
  "feature_indices",
...
static auto stumpMachine_load_doc = xbob::extension::FunctionDoc(
  "load",
...
static auto stumpMachine_save_doc = xbob::extension::FunctionDoc(
  "save",
...
PyObject* stumpMachineCreate(boost::shared_ptr<WeakMachine> machine){
  PyObject* o = StumpMachineType.tp_alloc(&StumpMachineType,0);
  reinterpret_cast<StumpMachineObject*>(o)->base = boost::dynamic_pointer_cast<StumpMachine>(machine);
...
bool init_StumpMachine(PyObject* module)
{
  ...
  // register machine
  if (!registerMachineType(typeid(StumpMachine).hash_code(), &stumpMachineCreate))
    return false;
